...
def _add_mode_metrics(frame: pd.DataFrame, mode: str) -> pd.DataFrame:
    ...
    if "square_feet" in enriched.columns:
        ...
        enriched["price_per_sqft"] = (
            pd.to_numeric(enriched["asking_price"], errors="coerce") / sqft.where(sqft > 0)
        ).round(2)
    if mode == "Land" and "lot_size" in enriched.columns:
        ...
        enriched["price_per_acre"] = (
            pd.to_numeric(enriched["asking_price"], errors="coerce")
            / enriched["acres"].where(enriched["acres"] > 0)
        ).round(0)
...
def _avm_request(row: pd.Series, radius: float, days: int, count: int) -> RentCastAvmRequest:
    return RentCastAvmRequest(
        ...
        square_feet=float(row["square_feet"]) if pd.notna(row.get("square_feet")) else None,
...
def _show_comps(comps: pd.DataFrame) -> None:
    if comps.empty:
        st.info("No comparable listings were returned.")
        return
    cols = [
        c
        for c in [
    ...
    st.dataframe(comps[cols], width="stretch", hide_index=True)
...
with c1:
    city_choice = st.selectbox(
        "Search area",
        ["Galesburg", "Canton", "Brimfield", "Kickapoo", "Peoria", "Custom city", "ZIP code"],
    )
with c2:
    state = st.text_input("State", value="IL", max_chars=2)
with c3:
    max_price = st.number_input(
        "Maximum asking price ($)", min_value=1000.0, value=50000.0, step=1000.0
    )
city, zip_code = "", ""
...
with c1:
    limit = st.number_input(
        "Maximum listings to retrieve", min_value=1, max_value=500, value=100, step=25
    )
with c2:
    days_old = st.number_input(
        "Listed within last N days (0 = any)", min_value=0, value=0, step=7
    )

if property_mode == "Land":
    st.info(
        "Preliminary scores use listing facts only. Run valuation on the strongest candidates before treating any parcel as a deal. "
        "If RentCast has no direct land AVM, Property Scout will try a comparable price-per-acre fallback. "
        "Zoning, access, utilities, flood risk, taxes, title, and buildability still require due diligence."
    )
...
if st.button(f"Search {property_mode.lower()} listings", type="primary"):
    ...
    try:
        ...
        if property_mode == "Land":
            st.session_state.land_valuations = {}
...
if st.session_state.live_search_completed and st.session_state.live_result_mode == property_mode:
    a, b, c = st.columns(3)
    a.metric("Live listings returned", st.session_state.live_source_count)
    b.metric("In buy box", len(intake_listings))
    c.metric("Filtered / invalid", len(rejected_listings))
    if st.session_state.live_source_count == 0:
        st.warning(
            f"No active {property_mode.lower()} listings matched this search. Try a higher maximum price or another nearby city/ZIP."
        )
    elif intake_listings.empty:
        st.warning("Listings were returned, but none passed the current type/price screen.")
...
if not intake_listings.empty:
    if property_mode == "Residential":
        ...
        valued = (
            pd.to_numeric(intake_listings.get("estimated_value"), errors="coerce").notna().sum()
            if "estimated_value" in intake_listings
            else 0
        )
        st.subheader("Land candidates — strongest screen first")
        st.caption(
            f"{valued} of {len(intake_listings)} candidates have valuation evidence. 'Best Deal' is reserved for valued candidates."
        )
        display_columns = [
            "deal_rating",
            "land_deal_score",
    ...
    st.download_button(
        ...
        intake_listings.to_csv(index=False).encode(),
    ...
    st.divider()
    st.header(
        "Land value + comparable review"
        if property_mode == "Land"
        else ("ARV + comparable review" if property_mode == "Residential" else "Commercial value + comparable review")
    )

    labels, indexes = [], []
    for idx, row in intake_listings.iterrows():
        asking = row.get("asking_price")
        label = f"{_address(row)} — ${float(asking):,.0f}" if pd.notna(asking) else _address(row)
        labels.append(label)
        indexes.append(idx)
    ...
    a, b, c = st.columns(3)
    with a:
        comp_radius = st.number_input(
            "Maximum comp radius (miles)", min_value=0.1, value=5.0, step=0.5
        )
    with b:
        comp_days = st.number_input(
            "Comparable lookback (days)", min_value=1, value=270, step=30
        )
    with c:
        comp_count = st.number_input(
            "Comparable count", min_value=5, max_value=25, value=15, step=1
        )

    if st.button("Get value estimate + comps"):
        request = _avm_request(selected, comp_radius, comp_days, comp_count)
        try:
            avm = fetch_rentcast_value_estimate(api_key, request)
            if property_mode == "Land":
                st.session_state.land_valuations[_address(selected)] = {
                    "estimated_value": avm.estimated_value,
                    "value_range_low": avm.range_low,
                    "value_range_high": avm.range_high,
                    "value_comp_count": avm.comp_count,
                    "value_source": "RentCast AVM",
                }
                asking = float(selected.get("asking_price") or 0)
                discount = (
                    (avm.estimated_value - asking) / avm.estimated_value * 100
                    if avm.estimated_value
                    else 0
                )
                st.success(
                    f"Valuation saved for reranking: estimated value ${avm.estimated_value:,.0f}, asking ${asking:,.0f}, estimated discount {discount:.1f}%."
                )
            a, b, c, d = st.columns(4)
            a.metric("RentCast value estimate", f"${avm.estimated_value:,.0f}")
            b.metric("Range low", f"${avm.range_low:,.0f}")
            c.metric("Range high", f"${avm.range_high:,.0f}")
            d.metric("Scout confidence", avm.confidence_label)
            st.caption(
                f"{avm.comp_count} comparable listing(s); estimate-range width: {avm.range_width_percent:.1f}%."
            )
            _show_comps(avm.comparables)
        except RentCastError as exc:
            if property_mode == "Land" and "non-positive" in str(exc).lower():
                st.warning(
                    "RentCast did not provide a usable direct AVM for this land listing. Trying a price-per-acre comparable fallback instead."
                )
                try:
                    comps = fetch_rentcast_comparables(api_key, request)
                    fallback = analyze_land_comps(selected.get("lot_size"), comps)
                    if fallback.estimated_value is not None:
                        st.session_state.land_valuations[_address(selected)] = {
                            "estimated_value": fallback.estimated_value,
                            "value_range_low": fallback.range_low,
                            "value_range_high": fallback.range_high,
                            "value_comp_count": fallback.usable_comp_count,
                            "value_source": "Land comp $/acre",
                        }
                        asking = float(selected.get("asking_price") or 0)
                        discount = (
                            (fallback.estimated_value - asking) / fallback.estimated_value * 100
                            if fallback.estimated_value
                            else 0
                        )
                        a, b, c, d = st.columns(4)
                        a.metric("Comp-derived land value", f"${fallback.estimated_value:,.0f}")
                        b.metric("Comp range low", f"${fallback.range_low:,.0f}")
                        c.metric("Comp range high", f"${fallback.range_high:,.0f}")
                        d.metric("Usable land comps", fallback.usable_comp_count)
                        st.success(
                            f"Fallback valuation saved for reranking. Median comparable price is about ${fallback.median_price_per_acre:,.0f}/acre; estimated discount to value is {discount:.1f}%."
                        )
                    else:
                        st.warning(
                            f"Direct AVM unavailable and only {fallback.usable_comp_count} usable price/lot-size comp(s) were returned. Property Scout will keep this candidate preliminary rather than invent a value."
                        )
                    _show_comps(fallback.comparables)
                except (ValueError, RentCastError) as fallback_exc:
                    st.error(f"Unable to retrieve usable land comps: {fallback_exc}")
            else:
                st.error(str(exc))
        except ValueError as exc:
            st.warning(str(exc))
